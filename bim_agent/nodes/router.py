...
async def force_terminate(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    # ── Simple text fallback for BIM queries ────────────────────────────
    if not partial_answer and llm_client:
        try:
            bim_metadata = (cfg.get("bim_metadata") if cfg else None)
            context = bim_metadata.to_text() if bim_metadata else ""
            fallback_prompt = (
                f"You are answering a question about a BIM building model.\n\n"
                f"Model summary:\n{context}\n\n"
                f"Question: {state['instruction']}\n\n"
                "Provide a brief best-effort answer based on the model summary above."
            )
            response = await llm_client.agenerate(
                messages=[{"role": "user", "content": fallback_prompt}],
            )
            partial_answer = response.content.strip()
            fallback_method = "llm_summary"
            logger.info("LLM summary fallback produced: %s", partial_answer[:200])
        except Exception as exc:
            logger.warning("LLM fallback failed: %s — using regex", exc)
    ...
    # Kernel stays running for potential reuse; caller shuts it down.
