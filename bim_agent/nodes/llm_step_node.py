...
from bim_agent.llm.react_response_schema import ReactResponseValidator
from bim_agent.llm.response_schema import LLMResponseValidator
...
async def llm_step_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            _openai_transient = ()
        is_transient = isinstance(exc, (*_openai_transient,
                                        ConnectionError, OSError, asyncio.TimeoutError))

        # Forgive the first 2 consecutive connection errors (transient network blip).
        # After that, count as a real failure so max_failures triggers force_terminate
        # instead of silently burning all max_steps.
        consecutive = state.get("consecutive_conn_failures", 0) + 1
        forgive = is_transient and consecutive <= 2
        return {
            "messages": [AIMessage(content=f"[LLM Error] {error_msg}")],
            "current_llm_response": None,
            "failure_count": state["failure_count"] if forgive else state["failure_count"] + 1,
            "consecutive_conn_failures": consecutive if is_transient else 0,
            "last_error_type": "llm_connection_error" if forgive else "llm_call_failed",
        }
