...
from bim_agent.kernel.variable_tracker import VariableTracker
from bim_agent.kernel.feedback_collector import (
...
def _build_multimodal_message(
    ...
    from bim_agent.llm.client import image_to_base64_url
...
async def feedback_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    # 6b. Warn the LLM as steps run low.
    steps_remaining = state["max_steps"] - (state["step_count"] + 1)
    if steps_remaining <= 0:
        feedback_text += (
            "\n\n[LAST STEP] This is your FINAL step. You MUST call ReturnAnswer(...) "
            "NOW with your best answer — the session ends after this step regardless."
        )
    elif steps_remaining <= 3:
        feedback_text += (
            f"\n\n[WARNING] Only {steps_remaining} step(s) remaining. "
            "Call ReturnAnswer(...) with your best current answer in this step — "
            "do not defer it."
        )
