...
def _resize_max_edge(img, max_edge: int):
    """Resize a PIL Image so its longest edge is at most max_edge."""
    w, h = img.size
    if max(w, h) <= max_edge:
        return img
    scale = max_edge / max(w, h)
    return img.resize((int(w * scale), int(h * scale)), resample=2)  # BILINEAR
...
class FrameImage:
    ...
    def image(self):
        if self._image is None and self._path is not None:
            ...
            if self._max_edge:
                img = _resize_max_edge(img, self._max_edge)
            self._image = img
        return self._image
