...
class ReturnAnswer:
    ...
    def __init__(self, answer):
        ...
        if isinstance(answer, (list, dict)):
            answer = _json.dumps(answer, ensure_ascii=False)
        elif not isinstance(answer, (str, int, float, bool)):
            raise TypeError(
                f"ReturnAnswer accepts str, int, float, dict, or list, got {type(answer).__name__}."
            )
